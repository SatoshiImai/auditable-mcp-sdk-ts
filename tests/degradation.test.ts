...
import { Posture, transportFor, UnnegotiatedCallError } from '../src/degradation';
...
describe('degradation postures (§6.2)', () => {
  ...
  it('a degraded session keeps recording, and the chain is uncountersigned (§5.2)', async () => {
    ...
    const session = new AmcpSession(chosen, SESSION, { deps: new FixedDeps() });
...
describe('a tool that requires a countersign (§5.2, §6.2)', () => {
  const requiring: AuditCapability = { ...L1, countersign: Countersign.HOST };
